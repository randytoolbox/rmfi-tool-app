...
// Checks open positions: sell at +5%, −3%, or after 5 trading days.
// Sends daily portfolio report email via Resend.

const ALPACA_BASE = 'https://paper-api.alpaca.markets';
const TAKE_PROFIT = 0.05;   // sell at +5%
const STOP_LOSS   = -0.03;  // sell at −3%
const MAX_DAYS    = 5;      // sell after 5 trading days
...
async function getBuyDates() {
  const orders  = await alpaca('/v2/orders?status=filled&direction=desc&limit=100');
  const buyDates = {};
  for (const o of (orders || [])) {
    if (o.side !== 'buy') continue;
    const m = (o.client_order_id || '').match(/^randy-(.+)-(\d{4}-\d{2}-\d{2})$/);
    if (m && !buyDates[o.symbol]) buyDates[o.symbol] = m[2];
  }
  return buyDates;
}
...
function buildEmail(positions, sold, account, dateStr) {
  ...
  const posCards = positions.length ? positions.map(p => {
    ...
    return `
      <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;padding:14px 16px;margin-bottom:10px;">
        <div style="font-size:18px;font-weight:700;color:#1e3a5f;">${p.symbol}</div>
        <div style="font-size:12px;color:#6b7280;">${p.qty} shares · entry ${fmt(p.avg_entry_price)}</div>
  ...
  }).join('') : `<div style="color:#6b7280;font-size:13px;padding:10px 0;">No open positions — will buy tomorrow morning.</div>`;
...
<body style="margin:0;padding:0;background:#fff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <div style="max-width:540px;margin:0 auto;padding:24px 16px;">
    ...
    <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:16px 20px;margin-bottom:18px;">
      <div style="font-size:12px;color:#6b7280;text-transform:uppercase;letter-spacing:.05em;">Open Portfolio Value</div>
    ...
    <div style="font-size:14px;font-weight:700;color:#374151;margin-bottom:8px;">Open Positions</div>
    ${posCards}
    ${soldSection}
    <div style="font-size:11px;color:#9ca3af;text-align:center;margin-top:20px;padding-top:16px;border-top:1px solid #f3f4f6;">
      Paper trading only — no real money. Rules: sell at +5% / −3% / 5 trading days.<br><br>
...
async function main() {
  ...
  for (const pos of positions) {
    ...
    const daysHeld = buyDates[pos.symbol] ? tradingDaysSince(buyDates[pos.symbol]) : 0;

    let reason = null;
    if (plpc >= TAKE_PROFIT)       reason = `Take profit at ${fmtPct({ valueOf: () => plpc })} ✅`;
    else if (plpc <= STOP_LOSS)    reason = `Stop loss at ${(plpc*100).toFixed(2)}% 🛑`;
    else if (daysHeld >= MAX_DAYS) reason = `${daysHeld} trading days — time's up ⏱`;

    if (reason) {
      try {
        await alpaca(`/v2/positions/${pos.symbol}`, { method: 'DELETE' });
      ...
      console.log(`HOLD ${pos.symbol}: ${(plpc*100).toFixed(2)}%, day ${daysHeld}`);
    }
  }
  ...
  const html = buildEmail(remaining, sold, account, dateStr);
