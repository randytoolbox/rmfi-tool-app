// Runs daily at 7:45am ET (Mon–Fri) via Vercel cron (11:45 UTC).
// Fetches stock data, scores picks, sends email via Resend.

const TO_EMAIL   = 'r22862550@gmail.com';
const FROM_EMAIL = 'onboarding@resend.dev';

// Core watchlist — edit this list to match what you track in the app
const WATCHLIST = [
  'SPY','QQQ','DIA','IWM',
...
module.exports = async function handler(req, res) {
  // Allow manual trigger via GET/POST as well as cron
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) return res.status(500).json({ error: 'RESEND_API_KEY not set' });

  // Fetch all stocks in parallel, drop failures
  ...
  if (!picks.length) return res.status(200).json({ status: 'no picks available' });
  ...
  if (!emailRes.ok) return res.status(502).json({ error: result });

  return res.status(200).json({
    status: 'sent',
    picks:  picks.map(d => d.symbol),
    emailId: result.id,
  });
};
